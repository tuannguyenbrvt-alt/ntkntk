...
try {
    ...
    echo "=== DISSECTING QUIZ 124 ===\n";
    
    // 1. Get quiz questions rows
    $stmtQQ = $db->prepare("SELECT * FROM quiz_questions WHERE quiz_id = 124 ORDER BY sort_order ASC");
    $stmtQQ->execute();
    $qqRows = $stmtQQ->fetchAll();
    echo "Total rows in quiz_questions for quiz 124: " . count($qqRows) . "\n";
