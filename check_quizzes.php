...
try {
    ...
    echo "=== DISSECTING QUIZ 127 ===\n";
    
    // 1. Get quiz questions rows
    $stmtQQ = $db->prepare("SELECT * FROM quiz_questions WHERE quiz_id = 127 ORDER BY sort_order ASC");
    $stmtQQ->execute();
    $qqRows = $stmtQQ->fetchAll();
    echo "Total rows in quiz_questions for quiz 127: " . count($qqRows) . "\n";
